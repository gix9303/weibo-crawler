...
**描述:** 对于定时任务的父任务（`schedule_id == task_id`），返回该定时任务的聚合结果 ZIP。聚合逻辑在子任务/父任务成功结束后由后台预生成，下载端点本身**不再现场聚合**。

聚合 ZIP 结构（每个用户一个目录）大致为：

- `<昵称>_weibos_all.csv`：该用户所有微博汇总；
- `<昵称>_comments_all.csv`：该用户所有评论汇总；
- `<昵称>_all*.pdf`：按配置（可选按年拆分）导出的时间线 PDF；
- `img/`、`video/`、`live_photo/`：从各子任务目录汇总的图片、视频。

**行为:**

- 若缓存 ZIP 已存在（`weibo/_schedule_cache/schedule_results_<schedule_id>.zip`）：
  - 直接返回该 ZIP（`200 OK`，`application/zip`），即使此时有新的子任务正在运行，也始终返回上一份已完成的结果。
- 若缓存尚不存在：
  - 在一次请求中最多检查 3 次（每次等待约 2 秒）是否生成了缓存 ZIP；
  - 若仍未生成，则返回错误（`503 Service Unavailable`），提示「聚合结果尚未生成，正在准备中，暂不可下载，请稍后重试」；
  - 若已在配置中启用 PushDeer 通知，则同时发送一条提醒。
- 该端点不会在下载时触发聚合运算；如需修复或重新生成聚合结果，应通过重新运行相关任务或等待下一个子任务完成后由后台自动重建缓存。
